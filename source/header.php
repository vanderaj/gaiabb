...
include(ROOTINC.'global.inc.php');
include(ROOTINC.'constants.inc.php');
include(ROOTINC.'validate.inc.php');
include(ROOTINC.'functions.inc.php');
...
else if (strpos($useragent, 'MSIE') !== false && strpos($useragent, 'Opera') === false)
{
    $browser = 'ie';
    $bbcode_js = 'ie';
}
else if (strpos($useragent, 'Gecko') !== false && strpos($useragent, 'Konqueror') === false)
{
...
include(ROOTCLASS.'cache.class.php');
...
{
    $array = parse_url($CONFIG['boardurl']);
...
$lang_nalign = 'right';
$charset = 'ISO-8859-1';
include(ROOT.'lang/'.$self['langfile'].'.lang.php');
...
else if (!empty($self['theme']) && (int) $self['theme'] > 0)
{
...
if ($workdir === false || $workdir != $currentdir)
{
...
{
    $versionlong = '<br />Powered by <a href="http://www.GaiaBB.com" target="_blank"><strong>'.$versionshort.'</strong></a> ('.$alpha.''.$beta.''.$gamma.''.$sp.'), &copy; 2011-2013 The GaiaBB Group';
}
else
{
    $versionlong = '<br />Powered by <a href="http://www.GaiaBB.com" target="_blank"><strong>'.$versionshort.'</strong></a>, &copy; 2011-2013 The GaiaBB Group';
}
...
if ($CONFIG['forumjump'] == 'on' && $quickjump === false)
{
    $quickjump = forumJump();
