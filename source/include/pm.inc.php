...
class pmDAO
{
    ...
    public function insert_pm($to, $from, $to_uid, $from_uid, $type, $owner, $folder, $subject, $message, $isRead, $isSent, $usepmsig)
    {
        ...
        $result = $db->query("INSERT INTO " . X_PREFIX . "pm (msgto, msgfrom, msgto_uid, msgfrom_uid, type, owner, folder, subject, message, dateline, readstatus, sentstatus, usesig) VALUES ('$to', '$from', '$to_uid', '$from_uid', '$type', '$owner', '$folder', '$subject', '$message', '$onlinetime', '$isRead', '$isSent', '$usepmsig')");
        if ($result == false) {
            return false;
        }
        return $db->insert_id();
...
class pmModel
{
        ...
        if ($rcpt = $db->fetch_array($query)) {
            ...
            if (!in_array($self['username'], $ilist) || X_ADMIN) {
                ...
                if ($rcpt['emailonpm'] == 'yes' && $rcpt['status'] != 'Banned') {
                    ...
                    $tpl_keys = array('{TO}', '{FROM}', '{LINK}');
                    $tpl_values = array($rcpt['username'], $self['username'], $pmurl);
    ...
    public function send($pmid, $msgto, $subject, $message, $pmpreview)
    {
        ...
        } else if (isset($self['sig']) && !empty($self['sig'])) {
            $usesigcheck = $cheHTML;
            $pmcheckhtml = '<br /><input type="checkbox" name="usesig" value="yes" ' . $usesigcheck . ' /> ' . $lang['textusesig'];
        } else {
        ...
        if ($pmid > 0 && noSubmit('previewsubmit')) {
            ...
            $db->free_result($query);
        } else if (onSubmit('previewsubmit')) {
            if (empty($message)) {
    ...
    public function view($pmid, $folders)
    {
        ...
        if ($pm) {
            ...
            if ($pm['type'] == 'draft') {
                ...
                $delchecked = $cheHTML;
            } else if ($pm['msgfrom'] != $self['username']) {
            ...
            if (!empty($pm['filename']) && $CONFIG['pmattachstatus'] == 'on') {
                ...
                if ($CONFIG['attachimgpost'] == 'on' && ($extention == 'jpg' || $extention == 'jpeg' || $extention == 'gif' || $extention == 'png' || $extention == 'bmp')) {
                    if ($pm['fileheight'] != '' && $pm['filewidth'] != '') {
                        ...
                        } else if (($w_ratio * $pm['fileheight']) < $CONFIG['max_attheight']) {
    ...
    public function pm_print($pmid, $eMail = false)
    {
        ...
        if ($pm) {
            ...
            if ($eMail) {
                $tpl_keys = array('{TO}', '{FROM}', '{MSG}', '{SENT}', '{FOLDER}', '{SUBJECT}');
                $tpl_values = array($pmto, $pmfrom, $pmmessage, $pmdateline, $pmfolder, $pmsubject);
    ...
    public function viewFolderList()
    {
        ...
        if (isset($_SESSION['folder'])) {$folder = $_SESSION['folder'];} else { $folder = '';}
        ...
        sort($folders);
        $folders = array_merge(array('Inbox' => $lang['textpminbox'], 'Outbox' => $lang['textpmoutbox']), $folders, array('Drafts' => $lang['textpmdrafts'], 'Trash' => $lang['textpmtrash']));
