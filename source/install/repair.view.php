    ...
    <tr>
        <td class="tablerow" bgcolor="#ffffff"><br>
            <form action="index.php" method="post">
                <input type="checkbox" name="summary" value="on" checked>&nbsp;Display
                summary of admin/mod users
                </p>
                <input type="checkbox" name="resttemp" value="on">&nbsp;Restore
                templates
                </p>
                <input type="checkbox" name="resetset" value="on">&nbsp;Reset to
                factory settings
                </p>
                <input type="checkbox" name="config" value="on" <?php echo $emerg ?>>&nbsp;Re-create
                config.php<?php echo $instr ?></p>
                <input type="checkbox" name="createsa" value="on" <?php echo $emerg ?>>&nbsp;Create Super Administrator
                account<?php echo $instr ?></p>
                <input type="checkbox" name="disablesa" value="on"
                    <?php echo $emerg ?>>&nbsp;Disable all other admins/mods<?php echo $instr ?></p>
                <input type="hidden" name="path" value="<?php echo $path ?>"> <input
                        type="hidden" name="step" value="repair"> <INPUT TYPE="submit"
                                                                         VALUE="Continue &gt;"/>
            </form>
        </td>
    </tr>
...
function view_admins($db, $tablepre)
{
    ...
    <table border="0" cellspacing="1px" cellpadding="5px" width="100%">
        ...
        if ($query !== false && $db->numRows($query) > 0) {
            while (($user = $db->fetchArray($query)) != false) {
