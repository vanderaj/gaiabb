...
class xmb19x extends convert
{
    ...
    */
    function settings()
    {
        ...
        $settings_sql = "UPDATE `".X_PREFIX."settings` SET ".
        "bbname='".$this->toDbHost->escape($oldsettings['bbname'])."', ".
        "sitename='".$this->toDbHost->escape($oldsettings['sitename'])."', ".
        "bboffreason='".$this->toDbHost->escape($oldsettings['bboffreason'])."', ".
        "bbrulestxt='".$this->toDbHost->escape($oldsettings['bbrulestxt'])."', ".
        "siteurl='".$this->toDbHost->escape($oldsettings['siteurl'])."', ".
        "indexnewstxt='".$this->toDbHost->escape($oldsettings['tickercontents'])."', ".
        "sitename='".$this->toDbHost->escape($oldsettings['sitename'])."', ".
        "adminemail='".$this->toDbHost->escape($oldsettings['adminemail'])."', ".

        // Integers
        "postperpage='".intval($oldsettings['postperpage'])."', ".
        "topicperpage='".intval($oldsettings['topicperpage'])."', ".
        "hottopic='".intval($oldsettings['hottopic'])."', ".
        "u2uquota='".intval($oldsettings['u2uquota'])."', ".

        // on / off (XMB has many varients on this, but we try to do as best we can
        "whosonlinestatus='".$this->valOnOff($oldsettings['whosonlinestatus'])."', ".
        "regstatus='".$this->valOnOff($oldsettings['regstatus'])."', ".
        "regviewonly='".$this->valOnOff($oldsettings['regviewonly'])."', ".
        "hideprivate='".$this->valOnOff($oldsettings['hideprivate'])."', ".
        "emailcheck='".$this->valOnOff($oldsettings['emailcheck'])."', ".

        "searchstatus='".$this->valOnOff($oldsettings['searchstatus'])."', ".
        "faqstatus='".$this->valOnOff($oldsettings['faqstatus'])."', ".
        "memliststatus='".$this->valOnOff($oldsettings['memliststatus'])."', ".
        "avastatus='".$this->valOnOff($oldsettings['avastatus'])."', ".
        "bbrules='".$this->valOnOff($oldsettings['bbrules'])."', ".
        "coppa='".$this->valOnOff($oldsettings['coppa'])."', ".
        "sigbbcode='".$this->valOnOff($oldsettings['sigbbcode'])."', ".
        "reportpost='".$this->valOnOff($oldsettings['reportpost'])."', ".
        "bbinsert='".$this->valOnOff($oldsettings['bbinsert'])."', ".
        "smileyinsert='".$this->valOnOff($oldsettings['smileyinsert'])."', ".
        "doublee='".$this->valOnOff($oldsettings['doublee'])."', ".
        "editedby='".$this->valOnOff($oldsettings['editedby'])."', ".
        "dotfolders='".$this->valOnOff($oldsettings['dotfolders'])."', ".
        "attachimgpost='".$this->valOnOff($oldsettings['attachimgpost'])."', ".
        "topicactivity_status='".$this->valOnOff($oldsettings['todaysposts'])."', ".
        "stats='".$this->valOnOff($oldsettings['statsstatus'])."'";
